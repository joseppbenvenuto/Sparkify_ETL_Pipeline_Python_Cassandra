...
def process_data(file_path, session):
    ...
    with open(file, 'r', encoding = 'utf8') as f:
        ...
        for line in csvreader:
            ...
            session.execute(table_2_3_1_1_insert,(line[0],line[9],float(line[5]),int(line[8]),int(line[3])))
            # Give me only the following: name of artist, song (sorted by itemInSession) and user 
            # (first and last name) for userid = 10, sessionid = 182
            session.execute(table_2_3_2_2_insert,(line[0],line[9],line[1],line[4],int(line[3]),int(line[10]),int(line[8])))
            # Give me every user name (first and last) in my music app history who listened to the 
            # song 'All Hands Against His Own'
            session.execute(table_2_3_3_3_insert,(line[1],line[4],line[9],int(line[10])))
    print('Inserted data for table_2_3_1_1, table_2_3_2_2, table_2_3_3_3')
